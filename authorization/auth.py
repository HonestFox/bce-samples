...
def normalize_string(in_str, encoding_slash=True):
    ...
    in_str = in_str.encode(DEFAULT_ENCODING) if isinstance(
        in_str, unicode) else str(in_str)
...
def get_canonical_querystring(params):
    ...
    result = ['%s=%s' % (k, normalize_string(v))
              for k, v in params.items() if k.lower != AUTHORIZATION]
...
def sign(credentials, http_method, path, headers, params,
    ...
    sign_result = hmac.new(sign_key, string_to_sign,
                           hashlib.sha256).hexdigest()
...
if __name__ == "__main__":
    ...
    body = None
    print result
    print get_canonical_time(timestamp)

    port = 80
    httpClient = None
    try:
        httpClient = httplib.HTTPConnection(host, port)
        httpClient.request(method=http_method, url=path,
